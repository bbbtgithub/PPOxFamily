...
def calculate_perplexity(model: GPT2LMHeadModel, query: torch.Tensor, response: torch.Tensor) -> float:
    ...
    **Arguments:**
    ...
    total_input = torch.cat([query, response], dim=0).float()
    
    
    
    # 确保 total_input 的长度为 obs_dim
    obs_dim = 8  # 与 ActorCritic 中的 obs_dim 保持一致
    if total_input.size(0) < obs_dim:
        print("填充到 obs_dim")
        total_input = torch.nn.functional.pad(total_input, (0, obs_dim - total_input.size(0)))
    else:
        print("截断到 obs_dim")
        total_input = total_input[:obs_dim]
        
    print("total_input shape:", total_input.shape)  # 调试输出 total_input 的形状
        
        
    # Forward pass through the model to get logits
    logits, _ = model(total_input.unsqueeze(0))  # Add batch dimension
    print("logits shape:", logits.shape)  # 调试输出 logits 的形状
    

    # 如果 logits 是 [1, vocab_size]，说明只输出了最后一个 token 的 logits
    if logits.dim() == 2 and logits.size(0) == 1:  # Shape: [1, vocab_size]
        shifted_logits = logits.squeeze(0)  # Remove batch dimension, shape: [vocab_size]
        shifted_labels = total_input[-1:]  # 最后一个 token 的标签
    else:
        # 如果 logits 是 [1, sequence_length, vocab_size]
        start = query.shape[0]
        shifted_logits = logits.squeeze(0)[start:-1, :]  # Remove batch dimension
        shifted_labels = total_input[start+1:]
        
        # 如果 shifted_logits 是 1D 张量，添加 batch_size 维度
    if shifted_logits.dim() == 1:
        shifted_logits = shifted_logits.unsqueeze(0)  # Shape: [1, vocab_size]

        # 检查 shifted_labels 是否为空
    if shifted_labels.numel() == 0:
        print("Warning: shifted_labels is empty. Returning high perplexity.")
        return float('inf')
    
    # 确保 shifted_labels 是 1D 张量
    shifted_labels = shifted_labels.squeeze()  # Shape: [1]
    # Calculate cross-entropy loss
    loss_fct = torch.nn.CrossEntropyLoss()
    loss = loss_fct(shifted_logits, shifted_labels.long())
